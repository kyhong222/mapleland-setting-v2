...
- **로그인해도 비로그인 데이터는 사라지지 않는다.** 둘 다 이 기기에 남고, 저장 슬롯 화면에서
  `디스코드 계정` / `로컬` 탭으로 나란히 본다(§5).
...
로컬은 사라지지 않고 게스트 보관소에 남으며, 저장 슬롯 화면의 '로컬' 탭에서 그대로 보인다.
...
| 없음 | 없음 | 아무 일도 없음 |
| 없음 | 있음 | 계정 것을 받아 표시 |
| **있음** | **있음** | **계정 기준으로 표시** (이 기기 벌은 아래 게스트 보관소로) |
| **있음** | **없음** | **"이 기기의 세팅을 계정에 올릴까요?"** → `올리기` / `나중에` |

마지막 칸이 사실상 첫 로그인이다(계정이 비었다 = 아직 한 번도 올린 적 없다).
**묻는 건 이 한 경우뿐이다.** 나머지는 묻지 않고 계정 기준으로 연다.

`나중에`는 **계정을 빈 채로 시작**한다는 뜻이다. 동기화를 멈춰 두는 선택지도 있었지만, 그러면
`mlsv2:*`에 로컬이 그대로 남아 저장 슬롯의 '계정' 탭이 로컬을 계정인 것처럼 보여주게 된다.
로컬은 게스트 보관소에 그대로 있으니 '로컬' 탭에서 언제든 꺼낼 수 있다.

### 게스트 보관소 — 로그인해도 로컬은 사라지지 않는다

**비로그인 데이터와 계정 데이터는 둘 다 이 기기에 남는다.**

- 로그인하면 지금 `mlsv2:*`에 있는 게스트 벌을 `mlsv2:guest`로 옮기고, 화면은 계정 데이터로 채운다.
- 로그아웃하면 계정 데이터를 지우고 `mlsv2:guest`를 되돌린다. 로그인 전 상태 그대로 돌아온다.

**게스트 벌은 이미 있으면 덮지 않는다.** 로그인 상태로 새로고침하면 화면에 있는 건 계정 데이터인데,
그때 덮으면 게스트 벌이 계정 데이터로 바뀌어 영영 못 돌아온다. 저장은 로그인 전환 1회뿐이어야 한다.
`scripts/cloudSmoke.ts`가 이 불변식을 지킨다.

로그아웃 복원은 **로그인 → 로그아웃 전이에서만** 한다. 컴포넌트 언마운트(탭 닫기)에서도 도는
cleanup에 얹으면 탭을 닫을 때마다 데이터가 바뀐다. 그래서 `signOutCleanup()`과 `stopCloudSync()`를
나눠 뒀다.

### 저장 슬롯의 두 탭

옮기기 전용 화면을 따로 두지 않는다. 저장 슬롯 화면(`SlotManager`)에 탭을 둘 뒀다.

| 탭 | 로그인 중 | 비로그인 |
|---|---|---|
| **디스코드 계정** | 계정 슬롯. 불러오기·저장·삭제·이름변경 전부 | **잠김** — 로그인을 유도한다 |
| **로컬** | 게스트 벌. **불러오기만** | 이 기기 슬롯. 전부 가능 |

로그인 중 '로컬' 탭이 읽기 전용인 이유는, 그 내용이 `mlsv2:guest`에 얼어 있는 한 벌이라서다.
거기에 저장을 허용하면 로그아웃할 때 되돌아갈 기준이 흔들린다.

**로컬 → 계정 옮기기는 두 동작으로 끝난다:** '로컬' 탭에서 `불러오기` → '디스코드 계정' 탭의
빈 칸에 `저장`. 기존 조작 그대로라 새로 배울 게 없다.

Supabase 키가 없으면 탭 자체를 그리지 않는다 — 로컬 전용으로 쓰던 화면 그대로다.
...
src/components/SlotManager.tsx     저장 슬롯 (계정/로컬 탭)
```
...
- **Supabase 채택** — Neon/Turso/D1은 무료 용량이 더 크지만(5GB) 디스코드 OAuth를 직접 구현해야 해서
  ...
  지원하지 않아 커스텀 토큰 서버가 필요하다. 이 앱에는 서버 코드가 없고
...
- **불러오기 전용 화면 폐기** — 게스트 벌을 계정으로 옮기는 화면을 따로 만들었다가 접었다.
  저장 슬롯 화면에 탭 두 개를 두면 '불러오기 -> 저장'이라는 기존 조작으로 같은 일이 되고,
  비로그인 사용자에게는 잠긴 탭이 로그인 이유를 설명해 준다.
- **로그인 순간의 3지선다 폐기** — 처음엔 `계정 것 / 이 기기 것 / 병합` 셋을 물었는데, 첫 화면치고
  읽을 게 너무 많았다. `계정이 기준`으로 단순화하고 합치는 일은 저장 슬롯의 두 탭으로 미뤘다(§5).
