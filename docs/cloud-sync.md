...
1. **계정 데이터 사용** — 로컬을 서버 것으로 교체
2. **이 기기 데이터로 덮기** — 서버를 로컬 것으로 교체
3. **병합** — 손실이 0인 범위에서만:
   - 저장 슬롯: **서버에서 비어 있는 칸에만** 로컬 슬롯을 채운다(24칸 초과분은 안내 후 남겨둠)
   - 공용 인벤토리: 합집합. 인스턴스 id가 겹치면 새로 발급
   - 현재 작업 빌드: 사용자가 고른 쪽

덮어쓰기 직전에 `mlsv2:backup`에 전체 스냅샷 1벌을 남겨 복구 경로를 둔다(마지막 1개만 유지).
...
src/components/AuthButton.tsx TopBar
