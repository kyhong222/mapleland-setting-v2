...
(단 샤프아이즈와 바이퍼 스턴 마스터리는 예외다. 툴팁대로면 `-100`이지만 메랜 실적용이 표기값
그대로라 `offset`을 쓰지 않는다 — 둘 다 docs/plan.md §크리티컬에 실측 근거가 있다. 같은 툴팁 문구를
쓰는 크리티컬 샷/스로우/펀치는 `-100`이 맞으니 한꺼번에 고치지 말 것.)
