...
- `weaponTypes`가 붙은 버프(마스터리/엑스퍼트 + 무기 부스터)는 `weaponGateOk()`로 게이팅된다.
  ...
  무기 제한이 없는 부스터(윈드·매직)는 `weaponTypes`를 비워 둔다.
...
1. **시작** — `master`에서 `git pull`로 최신을 받은 뒤, 그 시점을 기준으로 새 브랜치를 만든다.
   (오래된 로컬 `master` 위에서 브랜치를 파지 않는다.)
2. **작업** — 구현 → 검수(`npm run typecheck`, 필요 시 스모크 스크립트) → 커밋.
3. **마무리** — PR을 올리는 것으로 해당 작업을 끝낸다. PR 생성 후 **로컬은 다시 `master`로 되돌려 둔다.**
...
git checkout master        # 마무리: 로컬을 master로 복귀
```

### 현재 브랜치가 작업중인지 판단하는 법

로컬이 `master`가 아닌 브랜치에 있다면, 그 브랜치가 **끝난 작업인지 진행중인 작업인지는 로컬 상태가
아니라 remote의 PR 머지 여부로 판단한다.** (마무리 후 master 복귀를 못 한 채 남아 있는 브랜치일 수 있다.)

```bash
gh pr status                      # 현재 브랜치의 PR 상태
gh pr list --state merged --head <branch>   # 머지됐는지 확인
```

- PR이 **머지됨** → 끝난 작업. 이어서 커밋하지 말고 `master`로 돌아가 pull한 뒤 새 브랜치를 판다.
- PR이 **열려 있음 / 없음** → 진행중인 작업. 그 브랜치에서 계속한다.
