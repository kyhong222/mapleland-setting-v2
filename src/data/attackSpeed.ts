 ...
 *
 * 값을 분당횟수가 아니라 **ms**로 들고 있는다. 원자료가 ms이고 30ms 눈금에 맞춰져 있어
 * (측정값 427개의 최대공약수 = 30), apm으로 저장하면 반올림 오차가 굳는다
 * (브랜디쉬 6속 840ms = 71.4회인데 72로 저장되던 식).
 *
...
const CURVES = {
   ...
   * 실측 apm(52·49·45·42·40·37·35)을 30ms 눈금에 스냅한 값이다. 원자료의 Dragon Roar 행과
   * 5속(1440 vs 1410) 한 눈금 차이라 같은 포효 모션으로 보인다.
   * (단 메랜 드래곤 로어는 내부 쿨타임으로 묶여 있어 이 곡선을 쓰지 않는다 — docs §6 #1)
   */
...
export const SKILL_SPEED: Record<number, SpeedSpec> = {
  ...
  1311006: { type: 'fixed', ms: 2000 }, // 드래곤 로어 — 내부 쿨타임이 있어 공속 무관 30/분 (docs §6 #1)
